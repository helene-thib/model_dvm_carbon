"""
Plot the results of the seasonal model from dvm_model_seasonal_v1.py
"""
import pandas as pd
...
path_R = path_to_results_K / 'results_R.txt'
path_G = path_to_results_K / 'results_G.txt'
path_C = path_to_results_K / 'results_C.txt'
path_RMR = path_to_results_K / 'results_M.txt'
path_Dg = path_to_results_K / 'results_Dg.txt'
path_K = path_to_results_K / 'results_K'

# Read files at the given respective paths
output_names = ['results_R.txt', 'results_G.txt', 'results_C.txt', 'results_Dg.txt', 'results_M.txt', 'results_K']
results_K = []
results_T = []
results_I = []
...
if bm_id == "bm_fish":
    path_Ceq = path_to_results_K / 'Ceq_fishK.txt'
    path_Req = path_to_results_K / 'Req_fishK.txt'
elif bm_id == "bm_crust":
    path_Ceq = path_to_results_K / 'Ceq_crust.txt'
    path_Req = path_to_results_K / 'Req_crust.txt'
else:
    path_Ceq = path_to_results_K / 'Ceq_ceph.txt'
    path_Req = path_to_results_K / 'Req_ceph.txt'
...
ax.set_ylabel('Carbon mass ($mgC$ $m^{-2}$ $h^{-1}$)')
textstr = '\n'.join((
    r'$\mathrm{Vmax (m/h)}=%.1f$' % (int(vmax), ),
    r'$\mathrm{µ (h^{-1})}=%.4f$' % (mu, ),
    r'$\mathrm{\alpha (h^{-1})}=%.2f$' % (coef_alpha, ),
...
axs[1].set_ylabel('Detritus [%]')
axs[0].set_ylabel('Detritus [$mgC$ $m^{-2}$]')
axs[0].legend(loc="upper left", prop={'size': 9})

name_det_tot = 'Det_tot_timeK.pdf'
plot_det_tot = path_to_plot / name_det_tot
plt.savefig(plot_det_tot)
plt.close()